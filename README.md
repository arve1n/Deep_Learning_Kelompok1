...
Pertama pada model yang diusulkanmenerapkan model sequential. Model Sequential adalah arsitektur model digunakan untuk tumpukan lapisan (layer) di mana setiap lapisan memiliki tepat satu tensor input dan satu tensor output. Selanjutnya kita import Conv2D untuk menjalankan operasi konvolusional pada gambar latih. Langkah selanjutnya adalah pooling. Pooling bertujuan untuk mereduksi ukuran gambar sebanyak mungkin, dalam lapisan ini kita mengecilkan para gambar tumpukan ke dalam ukuran yang lebih kecil. Selanjutnya dilakukan Batch Normalization untuk menormalisasi agar model yang dibuat lebih cepat dan lebih stabil. kemudian Global Average Pooling2D merupakan layer yang berguna untuk mengurangi ukuran matriks, pada global average pooling digunakan nilai rerata keseluruhan untuk mengurangi ukuran dari matriks yang digunakan.
