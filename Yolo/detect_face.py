...
def get_faces_bbox(detect_img,model,device):
    # Load model
    img_size = 640
    conf_thres = 0.6
    iou_thres = 0.5
    ...
    im0s = detect_img.copy()
    # Padded resize
    im = letterbox(im0s, new_shape=640)[0]
    # Convert
...
if __name__ == '__main__':
    ...
    ## runs this
    # py detect_face.py --weights yolov5m-face.pt --source 'D:/Andre/Pemrograman/Github/Face Mask/images/maksssksksss0.png' --exist-ok

    ## hacky way to get the faces
    # get_faces_bbox(model, opt.source, device, opt.project, opt.name, opt.exist_ok, opt.save_img, opt.view_img)
